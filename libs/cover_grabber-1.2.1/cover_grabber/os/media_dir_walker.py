...
class MediaDirWalker(object):
    def __init__(self, path, output, overwrite = False):
        ...
        if filenames:
            ...
            if filehandler:
                ...
                if filehandler.audio_files:
                    ...
                    if album_name:
                        cover_exists = self.check_cover_image_existence(self.output) # Does cover image already exist in the current directory?
                        ...
                        if image_url:
                            ...
                            self.download_image(dirname, image_url, album_name)
    ...
    def download_image(self, dirname, image_url, album):
        ...
        if ".png" in image_url.lower():
            cover_name = hashlib.sha224(album).hexdigest() + ".png"
        elif ".jpg" in image_url.lower():
            cover_name = hashlib.sha224(album).hexdigest() + ".jpg"
        elif ".jpeg" in image_url.lower():
            cover_name = hashlib.sha224(album).hexdigest() +".jpg"
        elif ".gif" in image_url.lower():
            cover_name = hashlib.sha224(album).hexdigest()+ ".gif"
        elif ".tif" in image_url.lower():
            cover_name = hashlib.sha224(album).hexdigest()+ ".tiff"
        elif ".tiff" in image_url.lower():
            cover_name = hashlib.sha224(album).hexdigest()+ ".tiff"
        elif ".svg" in image_url.lower():
            cover_name = hashlib.sha224(album).hexdigest()+".svg"
        else:
            return

        # Does cover.(png|jpg|gif|tiff|svg) already exist?  
        if os.path.exists(os.path.join(self.output, cover_name)):
            # If overwrite is set to True, then go ahead and re-download album cover
            if self.overwrite:
                self.do_download(self.output, image_url, cover_name)
            else:
